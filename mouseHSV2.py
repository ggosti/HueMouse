...
# read mask
obj = (cv.imread(directory+'/mask1.0.png')[:,:,0] > 0)
print('obj',obj.shape)
...
while True:
    ...
    #labeled_array, num_features = ndi.label(th)
    ...
    #cv.drawContours(th, contours, -1, (0,255,0), 3)
    intersection = 0
    for col,cont in enumerate(contours):
        blank = np.zeros(th.shape[0:2])
        image1 = cv.fillPoly(blank, [cont], 1)
        try:
            intersection2 = np.sum(np.logical_and(obj, image1))
        except ValueError:
            print('Remake mask:',obj.shape, ' match ',image1.shape)
            cv.imwrite(directory+'/frame'+str(capture.get(cv.CAP_PROP_POS_FRAMES))+'.png', frame)
        if intersection<intersection2:
            #print('intersection',intersection,intersection2,intersection<intersection2)
            blank = np.zeros(th.shape[0:2],dtype=th.dtype)
            objNew = cv.fillPoly(blank, [cont], 1)#( (col*100)%255, (col*200)%255, (col*300)%255 ))
    ...
    if np.sum(intersection) < 10:
        ...
        plt.imshow(objNew)
        plt.show()
    #print('dtype in',obj.dtype,np.sum(obj))
            #li = [i for i in range(100000)]
    #print('dtype',th.dtype)
    if np.sum(objNew)>0:
        cy = int(np.sum(yy*objNew)/np.sum(objNew))
        cx = int(np.sum(xx*objNew)/np.sum(objNew))
    th = cv.cvtColor(th,cv.COLOR_GRAY2RGB)
    obj = objNew
    objCol = cv.cvtColor(obj*200,cv.COLOR_GRAY2RGB)
    ...
    if np.sum(th)>0:
        ...
        cv.circle(objCol,(cx,cy), 5, (0,255,255), -1)
